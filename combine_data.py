...
def make_files(data_dir, sub_dir, q_file, sum_file, cont_file):
	...
	for d1 in os.listdir(os.path.join(data_dir, sub_dir)):
		...
		f_q = open(q, "r", encoding = "utf8")
		query_sent = f_q.read().splitlines()[0]

		f_c = open(c, "r", encoding = "utf8")
		f_s = open(s, "r", encoding = "utf8")

		for l1, l2 in zip(f_c, f_s):
...
def make_files_all_dicts(data_dir, dir_lists, name):
	...
	q_file = open((os.path.join(name ,  "query.txt")), "w", encoding = "utf8")
	c_file = open((os.path.join(name , "content.txt")), "w", encoding = "utf8")
	s_file = open((os.path.join(name , "summary.txt")), "w", encoding = "utf8")
